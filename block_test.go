...
func TestBlock(t *testing.T) {
	b := rddutil.NewBlock(&Block100000)
	...
	wantSha, err := rddwire.NewShaHashFromStr(wantShaStr)
	if err != nil {
	...
	for i, txSha := range wantTxShas {
		wantSha, err := rddwire.NewShaHashFromStr(txSha)
		if err != nil {
			t.Errorf("NewShaHashFromStr: %v", err)
		}
	...
	for i := 0; i < 2; i++ {
		...
		for j, tx := range transactions {
			wantSha, err := rddwire.NewShaHashFromStr(wantTxShas[j])
			if err != nil {
				t.Errorf("NewShaHashFromStr: %v", err)
			}
...
func TestNewBlockFromBytes(t *testing.T) {
	...
	b, err := rddutil.NewBlockFromBytes(block100000Bytes)
	if err != nil {
...
func TestNewBlockFromBlockAndBytes(t *testing.T) {
	...
	// Create a new block from the serialized bytes.
	b := rddutil.NewBlockFromBlockAndBytes(&Block100000, block100000Bytes)
...
func TestBlockErrors(t *testing.T) {
	...
	testErr := rddutil.OutOfRangeError(wantErr)
	...
	_, err = rddutil.NewBlockFromBytes(shortBytes)
	if err != io.EOF {
	...
	_, err = b.TxSha(-1)
	if _, ok := err.(rddutil.OutOfRangeError); !ok {
		t.Errorf("TxSha: wrong error - got: %v <%T>, "+
			"want: <%T>", err, err, rddutil.OutOfRangeError(""))
	}
