...
func TestBase58(t *testing.T) {
	...
	for x, test := range stringTests {
		tmp := []byte(test.in)
		if res := rddutil.Base58Encode(tmp); res != test.out {
	...
	for x, test := range hexTests {
		...
		if res := rddutil.Base58Decode(test.out); bytes.Equal(res, b) != true {
	...
	for x, test := range invalidStringTests {
		if res := rddutil.Base58Decode(test.in); string(res) != test.out {
